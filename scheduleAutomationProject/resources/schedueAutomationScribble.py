from numpy import add, uint
import win32com.client as win32
import pandas as pd
import datetime

projectFile = win32.Dispatch('MSProject.Application')
projectFile.FileOpen('C:/Users/iamto/Documents/Project2.mpp')
...
def tdateUpdate(row,tdate,inc,asPlannedSchedule):
    ...
    duration=durationStringToDays(inc.Duration[row])
        ...
        #return inc.Start[row] + pd.Timedelta(duration,unit='days')
        #print("after updating")
        #print(inc.Start[row])
        #print(inc.loc[row,'Start'])
        #print(inc.loc[row,'Start'] + pd.Timedelta(3,unit='days'))
    ...
    for i in incIndex:
        if(inc.Finish[i] <= tdate):
            ...
            if flag:
                ...
                #inc.Start[i]=inc.Finish[prd]
                #inc.Finish[i]=inc.Start[i]+ pd.Timedelta(durationStringToDays(inc.Duration[i]),unit='days')
...
#print(inc.Duration[4])
#updater('2021-07-07',inc,asPlannedSchedule)
# print(inc)
